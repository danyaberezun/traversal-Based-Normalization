...
(define (Ats exp)
  (match exp
    [`(,l :lambda ,args ,lnfs) (foldl (λ (acc x) (append x (Ats x))) '() lnfs)]
    [`(,l :@ ,lnfs) (cons l (foldl (λ (acc x) (append x (Ats x))) '() lnfs))]
    [`(,l : ,var ,lnfs) (foldl (λ (acc x) (append x (Ats x))) '() lnfs)]
    ))
(define (Lams exp)
  (match exp
    [`(,l :lambda ,args ,lnfs) (cons l (foldl (λ (acc x) (append x (Lams x))) '() lnfs))]
    [`(,l :@ ,lnfs) (foldl (λ (acc x) (append x (Lams x))) '() lnfs)]
    [`(,l : ,var ,lnfs) (foldl (λ (acc x) (append x (Lams x))) '() lnfs)]
    ))
(define (Bvars exp)
  (match exp
    [`(,l :lambda ,args ,lnfs) (append args (foldl (λ (acc x) (append x (Bvars x))) '() lnfs))]
    [`(,l :@ ,lnfs) (foldl (λ (acc x) (append x (Bvars x))) '() lnfs)]
    [`(,l : ,var ,lnfs) (foldl (λ (acc x) (append x (Bvars x))) '() lnfs)]
    ))
...
(define (Generate-code-GOTO1-2 lnf static-binders list)
  (match lnf
    ...
    [`(,l :data ,i)
     (append `(if (equal? have (quote ,l)) (PLAY data_ ,i \; continue point (quote ,l))
                  ,(if (null? list)
                       '(error 'ERROR)
                       (Generate-code-GOTO1-2 (car list) static-binders (cdr list)))))]
    ))
