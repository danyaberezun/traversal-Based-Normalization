...
It is easy to see that term in state in \eqref{i3}, $A'_{B[\Gamma']}[\underline{\lambda x}.\; e[\Gamma]]$, has the head redex $(\lambda x,\ B)$ (since context is already empty). Thus, a step of head reduction will results exactly in term $A_{\xcancel{B}}[\xcancel{\lambda x}.\; \underline{e}[\Gamma][x/B[\Gamma']]]$ that is equal to the first component of state \eqref{ii3}. Following \textbf{(I)} and \textbf{(II)}, if we will continue to apply $exp$ function then elliminating the same $\Delta$ in both cases will lead no effect on argument $B$ and underlined subtrees. Thus, $(\lambda x,\ B)$ is a head redex in $M_i$ and the expansion of state $(ii)$ is $M_{i+1}$. \hfill{\textbf{Qed}} \\ \\ 
...
\item $(BVar)$
  ...
  \begin{tikzpicture}[remember picture,overlay]
    ...
    {\color{brown}\draw[<-]   ($(14.north)+(0,0.5)$)
      node[above,align=left,black] {$i$}  -- (14);}
    {\color{green}\draw[->] (16.south) to[bend left] (12.south);}
    {\color{brown}\draw[<-]   ($(14.north)+(0.1,0.5)$)
...
\item $(Lam-Non-Elim)$
  ...
  \begin{tikzpicture}[remember picture,overlay]
    ...
    {\color{blue}\draw[->] (13.north) to[bend right] (12.north);}
    {\color{blue}\draw[->] (14.north) to[bend right] (12.north);}
  \end{tikzpicture}
  \\ \\
\item $(Lam-Elim)$
  ...
  \begin{tikzpicture}[remember picture,overlay]
    ...
    {\color{blue}\draw[->] (15.north) to[bend right] (12.north);}
  \end{tikzpicture}
\item $(App)$
