...
fib2 = "(\\ q . (\\ w .  w @ (\\ e . \\ r . r))  @ ((q @ (\\ t . ((\\ y .  \\ u .  \\ i . (i @ y) @ u)   @ ((\\ o .  o @ (\\ p .  \\ a . a))   @ t)) @ (((\\ s .  \\ d .  \\ f .  \\ g .  (s @ f) @ ((d @ f) @ g))     @ ((\\ h .  h @ (\\ j .  \\ k . k))   @ t)) @ ((\\ l .  l @ (\\ z . \\ x . z))    @ t)))) @ (((\\ c .  \\ v .  \\ b .  (b @ c) @ v)    @ (\\ n .  \\ m . m))  @ (\\ q1 .  \\ w1 .  q1 @ w1))))  @ (\\ s1 . \\ z1 . s1 @ (s1 @ (s1 @ (s1 @ (s1 @ z1)))))"
...
main = do
  ...
  case parse parseExpr "" (filter (not . isSpace) ex_5) of
    Left  msg  -> error $ show msg
    Right term -> putStrLn $ show $ normalize $ annotate term
  putStrLn $ "traversal for term " ++ ex_6
  case parse parseExpr "" (filter (not . isSpace) ex_6) of
    Left  msg  -> error $ show msg
    Right term -> putStrLn $ show $ normalize $ annotate term
  putStrLn $ "traversal for term " ++ ex_7
