...
travers (Tr tr@(x@(x_e, (b_x, (up_x, bi_x))):trs)) bv len hn = case x of
  ...
  (ULVar _ z, (_, (up_x, _))) ->
    ...
    in case elem z bv of
      -- (FVAR) rule
      False -> 
        if hn == False then travers (hideNodes (Tr tr) len) bv len True
        else if up_x' == 0 then Tr $ (x_e, (True, (up_x, bi_x))):trs
        else case drop (len - up_x') tr of
          tr'@((ULApp _ _ r, (_, (up_a, _))):_) ->
      ...
      True  -> case isBinderApplied (Tr tr) z len of
        Just (n, i) -> case up_x of -- trace ("True\n" ++ show n ++ "\n") $
          LUP _ ->
            case n of
        ...
        Nothing     -> --trace ("False\n") $
          if hn == False then travers (hideNodes (Tr tr) len) bv len True
                  ...
                  tr'@((ULApp _ _ r, (_, (up_a, _))):_) -> 
                    travers (Tr $ (r, (True, (toPause up_a, computeBP' r up_x'))):(x_e, (b_x, (up_x, bi_x))):trs)
                            bv (len+1) False
      ...
      | i /= 0 = if member i is
        then let
          ...
          in {-trace ("hideNode' BIP case member : " ++ show (Tr $ x : trs)) $ -}
            ...
            (t, l) = hideNodes' trs ((-) len 1) is
          in {-trace ("hideNode' BIP case otherwise : " ++ show (Tr $ x : trs) ++ " " ++ show is ++ " " ++ show i) $-}
            (x:t, l)
