...
travers (Tr tr@(x@(x_e, (b_x, (up_x, bi_x))):trs)) bv len hn = case x of
      ...
      False -> 
        if hn == False then travers (hideNodes (Tr tr) len) bv len True
          ...
          tr'@((ULApp _ _ r, (_, (up_a, _))):_) -> case r of
            ...
            ULAbs _ _ _ -> travers (Tr $ (r, (True, (PAUSE $ up2int up_a, computeBP' r up_x))):(x_e, (b_x, (LUP up_x, bi_x))):trs) bv (len+1) False
            --_           -> travers (Tr $ (r, (True, (    up_a, computeBP' r up_x))):(x_e, (b_x, (LUP up_x, bi_x))):trs) bv (len+1) False
            _           -> travers (Tr $ (r, (True, (toPause up_a, computeBP' r up_x))):(x_e, (b_x, (LUP up_x, bi_x))):trs) bv (len+1) False
      -- (BVar) rule
  ...
  (ULVar _ z, (_, (PAUSE up_x, _))) ->
    case elem z bv of
      -- (FVAR) rule
      False -> 
        if hn == False then travers (hideNodes (Tr tr) len) bv len True
        else if up_x == 0 then Tr $ (x_e, (True, (PAUSE up_x, bi_x))):trs
        else case drop (len - up_x) tr of
          tr'@((ULApp _ _ r, (_, (up_a, _))):_) -> case r of
            ULAbs _ _ _ -> travers (Tr $ (r, (True, (PAUSE $ up2int up_a, computeBP' r up_x))):(x_e, (b_x, (PAUSE up_x, bi_x))):trs) bv (len+1) False
            _           -> travers (Tr $ (r, (True, (toPause up_a, computeBP' r up_x))):(x_e, (b_x, (PAUSE up_x, bi_x))):trs) bv (len+1) False
      -- (BVar) rule
      True  -> case isBinderApplied (Tr tr) z len of
        Just (n, i) -> -- trace ("True\n" ++ show n ++ "\n") $
          case n of
            ULAbs _ _ _ -> travers (Tr $ (n, (True, (PAUSE up_x, computeBP n i))):tr) bv (len + 1) False
            _           -> travers (Tr $ (n, (True, (PAUSE up_x, computeBP n i))):tr) bv (len + 1) False
        Nothing     -> --trace ("False\n") $
          if hn == False then travers (hideNodes (Tr tr) len) bv len True
          else if up_x == 0 then Tr tr -- $ (x_e, (True, (LUP up_x, bi_x))):trs
               else case drop (len - up_x) tr of
