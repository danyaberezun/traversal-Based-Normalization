...
an effect of its application to normalization procedure.
...
The $spec$ equation for a normalizer program \textbf{NP} that is just a 
function from \lc\ to Traversals can be seen on the slide.

You can see that there is no external dynamic data and
that is a tradition for \lc\ that $\lambda$-term \lam{M} is self-contained.

So a question is \textbf{why break normalization into two stages?}

Well, $\dots$ , there are several reasons to perform it:
\be
\ii First, a specializer output $\mbox{NP}_M = \lsem spec \rsem (\mbox{NP},
  M)$ can be in a \blue{much simplier language} than $\lambda$-calculus. 
  And our candidate for it is some \textbf{\brown{low-level language}}, 
  \lam{LLL}.

\ii Second, two stages will be natural for \blue{\em semantics-directed 
  compiler generation}.\\
  Our \green{aim is to use \lam{LLL} as an intermediate language to express 
  \red{semantics}}. This means that programs on this low-level language can 
  be thought as a \red{semantics} for programs from $\lambda$-calculus. 
  ...
  LLL$\\
  and the second stage, $\mbox{NP}_2$ is the \emph{semantic function} of 
  LLL-programs\\
...
A well-known fact is that partial evaluation will achive a best result if 
the input program is \emph{annotated}. Thus,

\be
\ii First, we have to \blue{annotate} parts of normalization procedure as 
  either \green{static} or \red{dynamic}. And here variables ranging over
  \be
  \ii \green{tokens} that are \green{static},
    Because there are only \blue{finitely many} subexpressions of \lam{$M$}.
    \\\\
    And all other data is actually dynamic
  \ii i.e. \red{back pointers} are \red{dynamic};
  \ii and so the \red{traversal} being built from both of them is
    \red{dynamic} too.
  \ee
\ii Then, run partial evaluator on annotated program. Computations in 
  normalization procedure \textbf{ONP} are either 
  \green{unfolded} by partial evaluator in compile time or \red{residualised}
  that means that partial evaluator will generate a runtime code to \red{do 
  them later} in run-time.\\
  Finaly,
  \bi
  \ii Perform all \green{fully static} computations  \green{at partial 
    evauation time}.
  \ii and for operations to build or test a traversal: generate \red{residual 
    code}.
  \ei
\ee
