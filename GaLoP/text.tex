...
\emph{head linear reduction}, which on each step substitutes only a \emph{head
variable occurence}, followed by regular \emph{head linear reductions} in all 
arguments.
...
traversals uses \red{\underline{no $\beta$-reductions}} at all. It leaves 
the original term intact, and can be implemented without \emph{traditional 
techniques} like environments, closures and so on.
...
constructs traversals using only \blue{syntax-derected rules}. While types are 
only used to contruct a correct \emph{long form} and to prove \lam{ONP} 
correctness.

And now we are able to formulate the \textbf{goals} of our research:
\bi
\ii The first goal is to extend \lam{ONP} to the untyped case (hereafter
  \lam{UNP});
...
\ii Most successful applications of parial evaluation are \blue{compilation} 
  and \blue{compiler generation}.
...
\ii Second, two stages are natural for \blue{\em semantics-directed 
  ...
  The first stage, denoted $\mbox{NP}_1$, is specialization of normalization 
  procedure on the input term \lam{$M$}
...
\ii First, we have to \blue{annotate} parts of normalization procedure as 
  ...
  many} subexpressions of a given term \lam{$M$}.
...
\bi
\ii First, the partial evaluator can do, at specialization time, \blue{all 
  of the \lam{ONP} operations, which depend only on input term \lam{$M$}} 
\ii \blue{wherein} this also means, that \lam{ONP$_M$} performs \green{
...
\ii \lam{UNP} is implemented in {\sc Haskell} and works for a variety of 
  examples. Right now we are working on a more \textbf{justified} definition 
  of \lam{UNP}.
...
\ii Moreover, by specializing \lam{UNP}, an \blue{arbitrary untyped $\lambda$-
  ...
  It actually means, that the specialized version of \lam{UNP} could be seen 
  as a semantic function for \lc.
